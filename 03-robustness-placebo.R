...
table_fun_pla <- function(crime_vec,
  ...
  if (is.null(add_lines_list)){
    add_lines_list <- 
      list(
        # c("Chief FE", rep(c("No",  "Yes", "Yes", "Yes"), n_blocks)),
...
# Table main
table_fun_pla(c('violent_death_sim_log',
                'vehicle_robbery_log',
                'street_robbery_log'),
              dep_var_labels = c("Violent deaths", 
                                 "Vehicle robbery (Carjacking)",	
                                 "Street robbery"),
              title = "Table A3 - Robustness: Effect of expectation of receiving bonuses on crime rates (Placebo analysis between 2005 and 2008)",          
